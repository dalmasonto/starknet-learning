...
#[generate_trait]
impl ShoppingCartImpl<T, impl TDrop: Drop<T>, impl TCopy: Copy<T>> of ShoppingCartTrait<T> {
    fn add_cart_item(ref self: ShoppingCart<T>, item: T) {
        self.cart_items.append(item);
    }
    ...
    fn get_cart_total<impl TAddEq: AddEq<T>>(self: @ShoppingCart<CartItem<T>>) -> u32 {
        let cart_items = self.cart_items;
        ...
        loop {
            ...
            let mut item: CartItem<u32> = *cart_items[counter];
            item.display_info();
            // total += item.price;
            counter += 1;
        };
...
fn main() {
    ...
    let mut item_1 = CartItem {
        name: 'Bread', price: 1, quantity: 1, category: ItemCategory::Groceries('Bakery')
    };
    ...
    // Get cart total here
    shopping_cart.get_cart_total().print();
